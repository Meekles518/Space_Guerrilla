...
// 기회주의형 적 Ai의 행동을 제어하는 스크립트
// 기회주의형 적 State들과 Enemy_Control 사이를 연결
public class Opportunistic_AI : MonoBehaviour
{
    Opportunistic_State currentState; // 현재 스테이트
    public Transform player; // 플레이어 트랜스폼
    public Enemy_Control control; // 컨트롤 컴포넌트

    static public bool isShoot; // Enemy_Shooter를 제어하는데 사용할 불 변수

    public Vector2 SelfSpawnposition; // 본인 스폰위치
    public Vector2 FleetSpawnpoint; // 전대스폰위치
    ...
    void OnEnable()
    {
        ...
        currentState = new Opportunistic_Idle(gameObject, player, control); // 초기 스테이트를 Idle로 설정
        ...
        // Oppotunistic 적에게 필요한 초기값들을 컨트롤 컴포넌트에 전달
